...
Lab.experiment('js only gets read from buildDirectory', function () {
    var tmpHash = crypto.randomBytes(16).toString('hex');
    var buildDir = path.join(os.tmpdir(), tmpHash);
    Lab.before(function (done) {
        async.series([
    ...
    Lab.test('css', function (done) {
        moonboots.cssSource(function (err, css) {
            Lab.expect(css).to.not.equal('cascading stylesheets!' + tmpHash);
            done();
        });
    });
});

Lab.experiment('css only gets read from buildDirectory', function () {
    var tmpHash = crypto.randomBytes(16).toString('hex');
    var buildDir = path.join(os.tmpdir(), tmpHash);
    Lab.before(function (done) {
        async.series([
    ...
    Lab.test('js', function (done) {
        moonboots.jsSource(function (err, js) {
            Lab.expect(js).to.not.equal('javascript!' + tmpHash);
            done();
        });
    });
    Lab.test('css', function (done) {
